...
class RabbitMQParserClient:
    def __init__(self, amqp_url: str, queue_name: str,
                 logger: TCPLogger, parser: ShortsParser):
                ...
                async with message.process():
                    if task_type == "channel":
                        ...
                        data = await self.parser.parse_channel(
                            url, channel_id, user_id, 3, proxy_list
                        )

                    # if task_type == "video":
                    #     self.logger.send("INFO", f"Начал парсить видео {url}")
                    #     data = await self.parser.parse_video(url, user_id)

            except Exception as e:
                self.logger.send("ERROR", f"❌ Ошибка обработки сообщения: {str(e)}")
                await message.ack()
