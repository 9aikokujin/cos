...
class InstagramParser:
    ...
    @staticmethod
    def _extract_www_claim_from_response(resp: httpx.Response) -> Optional[str]:
        for k in ("x-ig-set-www-claim", "x-ig-www-claim", "ig-set-www-claim"):
    ...
    @staticmethod
    def _dedupe_proxy(proxy: list[Optional[str]]) -> list[Optional[str]]:
    ...
    async def _validate_cookies(
        ...
        *,
        ...
        if not cookies.get("sessionid"):
            return False, None
        headers = self._build_headers(entry.get("user_agent"), cookies.get("csrftoken"), entry.get("www_claim"))
        proxy_for_httpx = self._format_proxy_for_httpx(proxy)
        client_kwargs: Dict[str, Any] = {"timeout": REQUEST_TIMEOUT, "headers": headers, "cookies": cookies}
        if proxy_for_httpx:
            if HTTPX_USES_PROXY_PARAM:
                client_kwargs["proxy"] = proxy_for_httpx
            else:
                client_kwargs["proxy"] = {"http": proxy_for_httpx, "https": proxy_for_httpx}
        try:
    ...
    async def ensure_initial_cookies(self, accounts: list[str]) -> Dict[str, Dict[str, Any]]:
        ...
        proxy_pool = self._dedupe_proxy(self.proxy_list or [])
        if proxy_pool:
        ...
        total_unique_proxy = len(proxy_pool)

        proxy_condition = asyncio.Condition()
        in_use_proxy: set[Optional[str]] = set()
        ...
        async def acquire_specific_proxy(
            ...
            async with proxy_condition:
                if proxy not in proxy_pool or proxy in tried:
                    return False
                if proxy in in_use_proxy:
                    return False
                in_use_proxy.add(proxy)
                return True
            ...
            async with proxy_condition:
                while True:
                    for proxy in proxy_pool:
                        if proxy not in in_use_proxy and proxy not in tried:
                            in_use_proxy.add(proxy)
                            return True, proxy
                    if len(tried) >= total_unique_proxy:
                        return False, None
        ...
        async def release_proxy(proxy: Optional[str]) -> None:
            async with proxy_condition:
                if proxy in in_use_proxy:
                    in_use_proxy.remove(proxy)
                    proxy_condition.notify_all()

        max_workers = min(total_unique_proxy or 1, MAX_PARALLEL_LOGIN_TASKS, len(accounts))
        ...
        async def process_account(account: str) -> None:
            ...
            if cached_entry:
                ...
                is_valid, status_code = await self._validate_cookies(cached_entry, proxy=last_proxy)
                if is_valid:
            ...
            tried_proxy: set[Optional[str]] = set()

            if last_proxy in proxy_pool:
                acquired_specific = await acquire_specific_proxy(last_proxy, tried_proxy)
                if acquired_specific:
                    entry_specific: Optional[Dict[str, Any]] = None
                    try:
                        entry_specific = await self._login_and_store_cookies(
                            username, password, two_factor_code, proxy_candidates=[last_proxy],
                        )
                    ...
                    tried_proxy.add(last_proxy)

                    if entry_specific:
                        await mark_valid(username, entry_specific)
                        return

            while len(tried_proxy) < total_unique_proxy:
                acquired, proxy = await acquire_proxy(tried_proxy)
                if not acquired:
                    break
                ...
                try:
                    entry = await self._login_and_store_cookies(
                        username, password, two_factor_code, proxy_candidates=[proxy],
                    )
                except InvalidCredentialsError as cred_exc:
                    print(f"⚠️ Пропускаем аккаунт {username}: {cred_exc}")
                    self._drop_session(username)
    ...
    async def _login_and_store_cookies(
        ...
        if cached_entry:
            try:
                is_valid, status_code = await self._validate_cookies(cached_entry, proxy=cached_entry.get("proxy"))
                if is_valid:
        ...
        proxy_pool = self._dedupe_proxy(proxy_candidates or self.proxy_list or [])
        ...
        for proxy_str in proxy_pool:
            ...
            try:
                ...
                browser = await playwright.chromium.launch(headless=False, args=["--window-size=390,844"])
                context_kwargs: Dict[str, Any] = {**(device or {}), "locale": "en-US"}
                ...
                login_result = await self.login_to_instagram(page, username, password, two_factor_code)
                if login_result:
                    cookies, www_claim = login_result
                    user_agent = await page.evaluate("navigator.userAgent")
                    entry = self._update_cookie_entry(username, cookies, user_agent, proxy_str, www_claim=www_claim)
    ...
    async def _request_with_sessions(
        ...
        *,
        ...
        for username, entry in session_items:
            proxy_for_httpx = self._format_proxy_for_httpx(entry.get("proxy"))
            headers = self._build_headers(
                entry.get("user_agent"),
                entry["cookies"].get("csrftoken"),
                entry.get("www_claim"),
            )
            client_kwargs: Dict[str, Any] = {"timeout": REQUEST_TIMEOUT, "headers": headers, "cookies": entry["cookies"]}
            if proxy_for_httpx:
                if HTTPX_USES_PROXY_PARAM:
                    client_kwargs["proxy"] = proxy_for_httpx
                else:
            ...
            try:
                ...
                new_claim = self._extract_www_claim_from_response(response)
                if new_claim and new_claim != entry.get("www_claim"):
                    entry["www_claim"] = new_claim
                    self.session_cache[username] = entry
                    self._persist_cookie_store()
                ...
                if status in (400, 429):
                    print(f"⚠️ Сессия {username} вернула {status}, пробуем другую.")
                    continue
            ...
            if status in (401, 403):
                ...
                if refreshed:
                    ...
                    return await self._request_with_specific_session(
                        sessions, username, refreshed, url, method=method, params=params, data=data
                    )
                sessions.pop(username, None)
                return await self._request_with_sessions(
                    sessions, url, method=method, params=params, data=data
                )

            if status in (400, 429):
                print(f"⚠️ Сессия {username} вернула {status}, переключаемся на другую.")
                return await self._request_with_sessions(
                    sessions, url, method=method, params=params, data=data
                )
    ...
    async def _fetch_reel_via_api(
        ...
        url = "https://www.instagram.com/graphql/query/" f"?doc_id={doc_id}&variables={quote(variables_str)}"
        ...
        if preferred_session and preferred_session[0] in sessions:
            response, session_username, entry = await self._request_with_specific_session(
                sessions, preferred_session[0], preferred_session[1], url,
            )
        else:
            response, session_username, entry = await self._request_with_sessions(sessions, url)
        ...
        *,
        ...
        while True:
            if max_pages is not None and pages_fetched >= max_pages:
                break
            pages_fetched += 1
            payload: Dict[str, Any] = {"target_user_id": user_id, "page_size": page_size}
            if next_max_id:
            ...
            if session_hint and session_hint[0] in sessions:
                response, session_username, session_entry = await self._request_with_specific_session(
                    sessions, session_hint[0], session_hint[1], clips_url, method="POST", data=payload,
                )
            else:
                response, session_username, session_entry = await self._request_with_sessions(
                    sessions, clips_url, method="POST", data=payload,
                )
    ...
    async def login_to_instagram(self, page, username, password, two_factor_code) -> Optional[Dict[str, str]]:
        api_errors = []
        captured_claim: Optional[str] = None

        async def log_response(response):
            nonlocal captured_claim
            if ("www.instagram.com" in response.url) or ("i.instagram.com" in response.url):
                try:
                    hdrs = response.headers
                    cl = hdrs.get("x-ig-set-www-claim") or hdrs.get("x-ig-www-claim")
                    if cl:
                        captured_claim = cl
                except Exception:
                    pass
        ...
        try:
            ...
            try:
                ...
                verification_code = await self.get_2fa_code(two_factor_code)
            ...
            if "/accounts/onetap/" in page.url or "/accounts/login/" not in page.url:
                ...
                if cookies:
                    # Вернём cookies и засечённый x-ig-www-claim (может быть None)
                    return (cookies, captured_claim)
    ...
    def _extract_metrics_from_media(self, shortcode_media: Dict[str, Any]) -> Dict[str, Any]:
        likes = (shortcode_media.get("edge_media_preview_like", {}).get("count"))
        comments = (
            shortcode_media
    ...
    async def parse_channel(
        ...
        try:
            clips_media, fetched_session = await self._fetch_user_clips(
                sessions, user_id, page_size=50, max_pages=None, preferred_session=preferred_session,
            )
        except Exception as exc:
        ...
        async def save_video_and_image(
            ...
            try:
                ...
                print(f"❌ Ошибка сохранения видео {reел_url}: {exc}")  # noqa
...
async def main():
    # ВСТАВЬТЕ СВОЙ СПИСОК ПРОКСИ СЮДА (убедитесь, что нет "слипшихся" строк — между элементами ДОЛЖНА быть запятая)
    proxy_list = [
