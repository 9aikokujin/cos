    ...
    async def get_all_videos_by_uid(self, uid: str, proxy_list: List[str]) -> List[Dict]:
        ...
        async with async_playwright() as p:
            ...
            while True:
                ...
                try:
                    self.logger.send("INFO", f"Делаем POST {api_url} c payload={payload}")
                    ...
                    if resp.status == 200:
                        ...
                        if data.get("code") == 0:
                            ...
                            if not videos:
                                self.logger.send("INFO", "→ Больше нет видео. Завершаем.")
                                break
                            all_videos.extend(videos)
                            if len(videos) < max_per_request:
                                self.logger.send("INFO", "→ Достигнут конец профиля.")
                                break
                            last_post_id = videos[-1].get("postId", "")
                            if not last_post_id:
                                self.logger.send("ERROR", "→ Нет lastPostId — завершаем.")
                                break
                        else:
