...
const SettingsSocial = ({ data }) => {
  ...
  // Каждый элемент теперь уникален по `id` или индексу
  const [socialsData, setSocialsData] = useState(
    data.socials.map((s, index) => ({
      ...s,
      localId: s.id || `${s.type}_${index}`, // уникальный идентификатор
      start_views: "",
      start_likes: "",
      start_comments: "",
      saved: false,
  ...
  // Универсальный апдейтер по localId
  const handleChange = (id, field, value) => {
    setSocialsData((prev) =>
      prev.map((item) =>
        item.localId === id ? { ...item, [field]: value } : item
      )
    );
  };
  ...
  const handleEdit = (id) => {
    setSocialsData((prev) =>
      prev.map((item) =>
        item.localId === id ? { ...item, saved: false } : item
      )
    );
  };

  const handleDelete = (id) => {
    setSocialsData((prev) => prev.filter((item) => item.localId !== id));
  };
  ...
  const handleSubmitAll = async () => {
    try {
      ...
      // Собираем все соцсети
      const socialsArray = socialsData.map((s) => ({
        type: s.type,
        link: s.link,
        start_views: Number(s.start_views) || 0,
        start_likes: Number(s.start_likes) || 0,
      ...
      await Promise.all(
        socialsArray.map((social) =>
          API.account.createAccount(social, data.user_id)
        )
      );
    ...
    <div className="modal_content" id="settingsSocial">
      ...
      <div className="social_wrap">
        {socialsData.map((s) => (
          <div
            key={s.localId}
            className="settings_social_container _flex_col_center"
          >
            <div className="social_name _flex_sb">
              <p className="_name">
                {s.type}
                {socialsData.filter((i) => i.type === s.type).length > 1 && (
                  <span style={{ opacity: 0.6 }}>
                    {" "}
                    #{s.localId.split("_").pop()}
                  </span>
                )}
              </p>
              <div className="_flex" style={{ gap: 10 }}>
            ...
            {!s.saved ? (
              ...
              >
                <Input
                  ...
                  value={s.start_views}
                  onChange={(e) =>
                    handleChange(s.localId, "start_views", e.target.value)
                  }
                />
                <Input
                  placeholder="Комментарии"
                  ...
                  value={s.start_comments}
                  onChange={(e) =>
                    handleChange(s.localId, "start_comments", e.target.value)
                  }
                />
                <Input
                  placeholder="Лайки"
                  ...
                  value={s.start_likes}
                  onChange={(e) =>
                    handleChange(s.localId, "start_likes", e.target.value)
                  }
                />
                <Button type="submit" className="_light_brown _btn">
                  Сохранить
