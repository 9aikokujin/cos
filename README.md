...
# Какие технологии используются (без деталей)
- Backend: FastAPI, SQLAlchemy, Pydantic, APScheduler, pika (RabbitMQ), httpx, Alembic, PostgreSQL.
- Парсеры: Playwright (для открытия страниц), aio-pika (RabbitMQ), httpx/requests.
...
1) Пользователь заходит в Telegram mini‑app. Авторизация проходит через Telegram (initData в заголовке Authorization).
2) В бэкенде есть API `/api/v1`, где хранятся пользователи, каналы, прокси, аккаунты, видео и история просмотров/лайков/комментариев.
...
- По расписанию дважды в день: 12:00 и 21:00 МСК (APScheduler), с небольшим смещением между каналами.
- Instagram запускается пакетом; остальные соцсети идут по очереди, чтобы не бомбить сразу все каналы.
...
# Юридически
Сервис предназначен для законного мониторинга открытых данных и при добросовестном использовании не нарушает законы или правила площадок.
